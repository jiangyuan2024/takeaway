<template>
	<view style="padding: 20rpx;">
		<view class="box">
			<uni-forms :modelValue="user" :rules="rules" ref="formRef" label-width="120rpx" label-align="right">
				<uni-forms-item label="头像" name="avatar">
					<uni-file-picker limit="1" :image-styles="imageStyles" :del-icon="false" :disable-preview="true" 
						fileMediatype="image" v-model="userAvatar"  @select="handleAvatarUploadSuccess"></uni-file-picker>
				</uni-forms-item>
				<uni-forms-item label="用户名" name="username">
					<uni-easyinput type="text" v-model="user.username" disabled />
				</uni-forms-item>
				<uni-forms-item label="姓名" name="name">
					<uni-easyinput type="text" v-model="user.name" placeholder="请输入姓名" />
				</uni-forms-item>
				<uni-forms-item label="性别" name="sex">
					<uni-data-checkbox style="position: relative; top: 10rpx;" v-model="user.sex"
						:localdata="[{text: '男', value: '男'}, {text: '女', value: '女'}]"></uni-data-checkbox>
				</uni-forms-item>
				<uni-forms-item label="电话" name="phone">
					<uni-easyinput type="text" v-model="user.phone" placeholder="请输入电话" />
				</uni-forms-item>
				<uni-forms-item>
...
<script>
	export default {
		...
		onLoad() {
			this.userAvatar = { url: this.user.avatar }
		},
		methods: {
